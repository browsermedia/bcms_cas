...
class CasUtilsTest < ActiveSupport::TestCase
  ...
  test "service_url returns absolute success_url if specified" do
    ...
    assert_equal "http://localhost:3000/stuff", CasModule::Utils.service_url(@portlet, @current_page), "This really need to be an absolute path for it work w/ CAS."
  end
  ...
  test "cas_server_url looks up CAS server from config in environment" do
    ...
    assert_equal expected, CasModule::Utils.cas_server_url
  end

  test "get login ticket" do
    expected_host = "https://random.com"
    CasModule::Utils.expects(:cas_server_url).with.returns(expected_host)
    ...
    ticket = CasModule::Utils.fetch_lt_from_cas
  ...
  test "Cas::Utils#logout redirects and attaches gateway=true to logout_url" do
    ...
    CasModule::Utils.expects(:reset_session_and_get_referrer).returns(destination_url)
    
    CasModule::Utils.logout(controller, "/")
  end
end
